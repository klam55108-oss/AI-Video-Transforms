...
class GPT52Client:
    ...
    async def query(
        ...
        *,
        ...
        Returns:
            GPT52Response with the model's output
        """
        if not self.is_available():
            return GPT52Response(
                success=False,
                output="",
                error="OPENAI_API_KEY not configured. Add it to .env file.",
            )
...
# Lazy singleton
_client: GPT52Client | None = None


def get_client() -> GPT52Client:
    """Get or create the GPT-5.2 client singleton."""
    global _client
    if _client is None:
        _client = GPT52Client()
    return _client
